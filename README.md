...
- 直接拖动歌词卡片可以移动位置
- 拖动右下角手柄可以自由调整悬浮窗大小
- 点击 `Tune` 打开设置面板，支持字体预设、字号、透明度、文字偏移、吸附位置
- 点击设置面板右上角 `Close` 可关闭设置面板
- 调整后的大小、位置、透明度会在下次启动时自动保留
...
1. 增加更多同步歌词源（在保证稳定性前提下）继续提高命中率
2. 做歌词切换时的平滑过渡（减少突变感）
3. 增加“快捷键可配置”与“设置导入导出”
4. 增加多显示器场景下的窗口策略（记忆每个屏幕的位置偏好）
